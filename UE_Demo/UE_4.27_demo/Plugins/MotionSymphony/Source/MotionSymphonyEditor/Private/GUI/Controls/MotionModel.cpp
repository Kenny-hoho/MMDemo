...
FMotionModel::FMotionModel(FMotionAnimAsset* InMotionAnim, UDebugSkelMeshComponent* InDebugSkelMesh)
	: MotionAnim(InMotionAnim),DebugMesh(InDebugSkelMesh)
{
	SetViewRange(TRange<double>(0.0, MotionAnim->GetPlayLength()));
	SetPlaybackRange(TRange<double>(0.0f, MotionAnim->GetPlayLength()));
}

FMotionModel::FMotionModel(FMotionAnimAsset* InMotionAnim, UDebugSkelMeshComponent* InDebugSkelMesh, TArray<UDebugSkelMeshComponent*> InDebugSkelMeshs)
	: MotionAnim(InMotionAnim), DebugMesh(InDebugSkelMesh),InteractionDebugMeshs(InDebugSkelMeshs)
{
	SetViewRange(TRange<double>(0.0, MotionAnim->GetPlayLength()));
	SetPlaybackRange(TRange<double>(0.0f, MotionAnim->GetPlayLength()));
}
...
void FMotionModel::SetScrubPosition(FFrameTime NewScrubPosition) const
{
	...
	if (InteractionDebugMeshs.Num()>0)
	{
		for (int32 i = 0; i < InteractionDebugMeshs.Num(); i++)
		{
			if (InteractionDebugMeshs[i])
			{
				InteractionDebugMeshs[i]->PreviewInstance->SetPosition(NewScrubPosition.AsDecimal() / (double)GetTickResolution());
			}
		}
	}
}
