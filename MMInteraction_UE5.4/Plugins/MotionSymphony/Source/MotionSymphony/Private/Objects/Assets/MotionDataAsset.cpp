...
#include "Tags/Tag_Interaction.h"
...
void UMotionDataAsset::PreProcessAnim(const int32 SourceAnimIndex, const bool bMirror /*= false*/)
{
	...
	// compute EndPoseId
	int32 EndPoseId = StartPoseId + FMath::RoundHalfToEven((CurrentTime / PlayRate) / PoseInterval);
	...
	for (FAnimNotifyEvent& NotifyEvent : MotionAnim->Tags)
	{
		if (UTagSection* TagSection = Cast<UTagSection>(NotifyEvent.NotifyStateClass))
		{
			...
			/*-----------------XC: InteractPointPreprocess--------------------*/
			if (UTag_Interaction* TagInteract = Cast<UTag_Interaction>(TagSection)) {

				// Cache InteractPoint
				FVector InteractPointLocation = FVector(0, 0, 0);
				InteractPointLocation.X = LookupPoseMatrix.PoseArray[Poses[TagStartPoseId].PoseId * LookupPoseMatrix.AtomCount + 1];
				InteractPointLocation.Y = LookupPoseMatrix.PoseArray[Poses[TagStartPoseId].PoseId * LookupPoseMatrix.AtomCount + 2];
				InteractPointLocation.Z = LookupPoseMatrix.PoseArray[Poses[TagStartPoseId].PoseId * LookupPoseMatrix.AtomCount + 3];

				FTransform CurrentRootTransform = FTransform::Identity;
				UMotionSequenceObject* MotionSequence = Cast<UMotionSequenceObject>(MotionAnim);
				MotionSequence->Sequence->GetBoneTransform(CurrentRootTransform, FSkeletonPoseBoneIndex(0), Poses[TagStartPoseId].Time, false);
				// 将交互点初始位置从局部坐标转换到世界坐标
				// FVector InitialLocation_WS = UKismetMathLibrary::TransformDirection(CurrentRootTransform, FVector(0, 0, 100));
				FVector TagStartLocation_WS = CurrentRootTransform.GetLocation() + InteractPointLocation;

				// 处理Tag之前帧
				TagInteract->InteractPointInitialLocation = TagStartLocation_WS;
				for (int32 PoseIndex = StartPoseId; PoseIndex < TagStartPoseId; ++PoseIndex) {
					TagInteract->PreProcessPose(Poses[PoseIndex], MotionAnim, this, TagStartTime, TagEndTime);
				}

				// Cache InteractPoint
				InteractPointLocation.X = LookupPoseMatrix.PoseArray[Poses[TagEndPoseId].PoseId * LookupPoseMatrix.AtomCount + 1];
				InteractPointLocation.Y = LookupPoseMatrix.PoseArray[Poses[TagEndPoseId].PoseId * LookupPoseMatrix.AtomCount + 2];
				InteractPointLocation.Z = LookupPoseMatrix.PoseArray[Poses[TagEndPoseId].PoseId * LookupPoseMatrix.AtomCount + 3];
				MotionSequence->Sequence->GetBoneTransform(CurrentRootTransform, FSkeletonPoseBoneIndex(0), Poses[TagEndPoseId].Time, false);
				FVector TagEndLocation_WS = CurrentRootTransform.GetLocation() + InteractPointLocation;
				// 处理Tag结束后的帧
				TagInteract->InteractPointInitialLocation = TagEndLocation_WS;

				for (int32 PoseIndex = TagEndPoseId; PoseIndex < EndPoseId; ++PoseIndex) {
					TagInteract->PreProcessPose(Poses[PoseIndex], MotionAnim, this, TagStartTime, TagEndTime);
				}
