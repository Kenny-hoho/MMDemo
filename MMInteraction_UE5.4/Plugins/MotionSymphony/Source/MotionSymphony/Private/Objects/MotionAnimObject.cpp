...
UMotionAnimObject::UMotionAnimObject():
	AnimId(0),
	MotionAnimAssetType(EMotionAnimAssetType::None),
	AnimAsset(nullptr),
	bLoop(false),
...
UMotionAnimObject::UMotionAnimObject(UAnimationAsset* InAnimAsset, UMotionDataAsset* InParentMotionData)
	: AnimId(0),
	MotionAnimAssetType(EMotionAnimAssetType::None),
	bLoop(false),
	bEnableMirroring(false),
	...
	CostMultiplier(1.0f),
	ParentMotionDataAsset(InParentMotionData)
{
}
...
void UMotionAnimObject::GetInteractionAnimsRootBoneTransforms(FTransform& OutTransform, int32 interactionIndex, const double Time) const
{
	OutTransform = FTransform::Identity;
}

void UMotionAnimObject::CacheInteractionTrajectortPoints(TArray<TArray<FVector>>& OutTrajectoryPoints) const
{
	OutTrajectoryPoints.Empty();
}
...
UMotionSequenceObject::UMotionSequenceObject(const FObjectInitializer& ObjectInitializer)
	: UMotionAnimObject(ObjectInitializer), Sequence(nullptr)
{
	MotionAnimAssetType = EMotionAnimAssetType::Sequence;
}

UMotionSequenceObject::UMotionSequenceObject()
	: UMotionAnimObject(), Sequence(nullptr)
{
}

UMotionSequenceObject::UMotionSequenceObject(UAnimSequence* InSequence, UMotionDataAsset* InParentMotionData)
	: UMotionAnimObject(InSequence, InParentMotionData), Sequence(nullptr)
{
}

UMotionSequenceObject::UMotionSequenceObject(UAnimSequence* InSequence, UMotionDataAsset* InParentMotionData, TArray<UDebugSkelMeshComponent*> InInteractionDebugSkelMeshComponents)
{
...
void UMotionSequenceObject::GetInteractionAnimsRootBoneTransforms(FTransform& OutTransform, int32 interactionIndex, const double Time) const
{
	if (InteractionAnims.Num() == 0 || InteractionAnims[interactionIndex]==nullptr)
	{
		OutTransform = FTransform::Identity;
		return;
	}

	InteractionAnims[interactionIndex]->GetBoneTransform(OutTransform, FSkeletonPoseBoneIndex(0), Time, false);
	...
	//			InteractionAnims[i]->GetBoneTransform(OutTransforms[i], BoneIndex, Time, false);
	//		}
	//		else
	//		{
...
void UMotionSequenceObject::CacheInteractionTrajectortPoints(TArray<TArray<FVector>>& OutTrajectoryPoints) const
{
	if (InteractionAnims.Num() > 0)
	{
		for (int32 i = 0; i < InteractionAnims.Num(); i++)
		{
			if (InteractionAnims[i]) {
...
UMotionBlendSpaceObject::UMotionBlendSpaceObject()
	: UMotionAnimObject(),
	BlendSpace(nullptr),
	SampleSpacing(0.1f, 0.1f)
{
...
UMotionBlendSpaceObject::UMotionBlendSpaceObject(UBlendSpace* InBlendSpace, UMotionDataAsset* InParentMotionData)
	: UMotionAnimObject(InBlendSpace, InParentMotionData),
...
UMotionCompositeObject::UMotionCompositeObject()
	: UMotionAnimObject(), 
...
UMotionCompositeObject::UMotionCompositeObject(UAnimComposite* InComposite, UMotionDataAsset* InParentMotionData)
	: UMotionAnimObject(InComposite, InParentMotionData),
