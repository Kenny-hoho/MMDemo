...
FName UMatchFeature_Trajectory2D::GetFeatureName() const
{
	return FName("Traj2D");
}
                                                     ...
                                                     FPrimitiveDrawInterface* DrawInterface)
{
	...
	for(int32 i = 0; i < TrajectoryTiming.Num(); ++i)
	{
		...
		FVector UpOffset = FVector(0, 0, 30.0f);
		FVector DrawFrom = PointPos + UpOffset;

		DrawInterface->DrawLine(PointPos, DrawFrom, LinearDebugColor,
			SDPG_Foreground, 3.0f);
		...
		DrawDebugDirectionalArrow(World, DrawFrom, DrawFrom + ArrowVector, 20.0f, DebugColor,
			true, -1, -1, 1.5f);
	...
	FMotionMatchingInputData& InputData, const int32 FeatureOffset, UMotionMatchConfig* MMConfig)
{
	...
	for(int32 i = 0; i < TrajectoryTiming.Num(); ++i)
	{
		...
		FVector UpOffset = FVector(0, 0, 30.0f);
		FVector DrawFrom = PointPosition + UpOffset;
		FVector DrawTo = DrawFrom + MeshTransform.TransformVector(FacingOffset *
			FVector(InputArray[StartIndex+2], InputArray[StartIndex+3], 0.0f) / 3.0f);
		
		AnimInstanceProxy->AnimDrawDebugLine(PointPosition, DrawFrom,
			DebugColor, false, -1.0f, 2.0f, SDPG_Foreground);
		AnimInstanceProxy->AnimDrawDebugDirectionalArrow(DrawFrom, DrawTo, 40.0f, DebugColor,
