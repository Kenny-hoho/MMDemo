...
void UTrajectoryGenerator::UpdatePrediction(float DeltaTime)
{
	...
	if (TrajectoryControlMode == ETrajectoryControlMode::Interaction) {
		CalculateInputVectorForInteract(DesiredLinearVelocity);
	}
	else {
...
void UTrajectoryGenerator::CalculateInputVectorForInteract(FVector& OutVelocity) const
{
	float CurrentSpeed = MaxSpeed;
	const FVector CurrentActorLoc = FVector(OwningActor->GetActorLocation().X, OwningActor->GetActorLocation().Y, 0.0f);
	if (UInteractionGPComponent* InteractComp = OwningActor->GetComponentByClass<UInteractionGPComponent>()) {
		if (AInteractActor* CurrentInteractActor = Cast<AInteractActor>(InteractComp->CurrentHitActor)) {
			const FVector MoveToLoc = CurrentInteractActor->MoveToLocation;
			float Distance = (MoveToLoc - CurrentActorLoc).Size();
			if (Distance < SlowDownDistance) {
				CurrentSpeed = FMath::Max(((Distance - StopDistance) / (SlowDownDistance - StopDistance)) * MaxSpeed, 0.0f);
			}
		}
	}
	...
	OutVelocity = InputVector * CurrentSpeed;
}
