	...
	//: FEditorViewportClient(new FAssetEditorModeManager(), nullptr, InEditorViewportWidget)
{
	...
	SetInitialViewTransform(LVT_Perspective, FVector(-300.0f, 300.0f, 150.0f), FRotator(0.0f, -45.0f, 0.0f), 1.0f);
}
...
void FMotionPreProcessToolkitViewportClient::DrawCanvas(FViewport& InViewport, FSceneView& SceneView, FCanvas& Canvas)
{
	...
	/*----------------XC: Get Root Velocity and Interaction Point Info---------------*/
	TObjectPtr<const UMotionSequenceObject> SourceAnim = ActiveMotionData->GetSourceSequenceAtIndex(Pose.AnimId);

	FVector RootVelocity;
	float RootRotVelocity;
	FMMPreProcessUtils::ExtractRootVelocity(RootVelocity, RootRotVelocity, SourceAnim->Sequence, Pose.Time, ActiveMotionData->PoseInterval);


	int32 FeatureOffset = 1;
	bool HasInteractFeature = false;
	for (UMatchFeatureBase* Feature : ActiveMotionData->MotionMatchConfig->Features) {
		if (Cast<UMatchFeature_Interaction>(Feature)) {
			HasInteractFeature = true;
			break;
		}
		FeatureOffset += Feature->Size();
	}
	TArray<float>& PoseArray = ActiveMotionData->LookupPoseMatrix.PoseArray;
	const int32 StartIndex = PreviewIndex * ActiveMotionData->LookupPoseMatrix.AtomCount + FeatureOffset;
	FVector InteractPointLoc = FVector(PoseArray[StartIndex], PoseArray[StartIndex + 1], PoseArray[StartIndex + 2]);

	/*----------------XC: Get Root Velocity and Interaction Point Info---------------*/
	FText PoseText;
	if (HasInteractFeature) {
		PoseText = FText::Format(LOCTEXT("PoseText", "Anim Name: {5}\nPose Id: {0} \nAnim Id: {1} \nMirrored: {6}  \nLast Pose Id: {2} \nNext Pose Id: {3}  \nCost Multiplier: {4} \nRoot Velocity: {7} \nInteactPointLoc.X: {8}\nInteactPointLoc.Y: {9}\nInteactPointLoc.Z: {10}"),
			Pose.PoseId, Pose.AnimId, Pose.LastPoseId, Pose.NextPoseId, ActiveMotionData->GetPoseFavour(Pose.PoseId),
			MotionPreProcessToolkitPtr.Pin()->CurrentAnimName, Pose.bMirrored, RootVelocity.Size(), InteractPointLoc.X, InteractPointLoc.Y, InteractPointLoc.Z);
	}
	else {
		PoseText = FText::Format(LOCTEXT("PoseText", "Anim Name: {5}\nPose Id: {0} \nAnim Id: {1} \nMirrored: {6}  \nLast Pose Id: {2} \nNext Pose Id: {3}  \nCost Multiplier: {4} \nRoot Velocity: {7}"),
			Pose.PoseId, Pose.AnimId, Pose.LastPoseId, Pose.NextPoseId, ActiveMotionData->GetPoseFavour(Pose.PoseId),
			MotionPreProcessToolkitPtr.Pin()->CurrentAnimName, Pose.bMirrored, RootVelocity.Size());
	}
...
void FMotionPreProcessToolkitViewportClient::Tick(float DeltaSeconds)
{
	...
	int32 InteractionCompNum = InteractionAnimatedRenderComponents.Num();
	TObjectPtr<UMotionAnimObject> CurrentMotionAnim = MotionPreProcessToolkitPtr.Pin().Get()->GetCurrentMotionAnim();
	if (CurrentMotionAnim) {
		int32 InteractionAnimNum = CurrentMotionAnim->InteractionAnims.Num();
		// 当前组件个数不等于交互动画个数，要更新交互组件
		if (InteractionAnimNum != InteractionCompNum) {
			TArray<USkeleton*> InteractionSkeletons;
			InteractionSkeletons.SetNum(CurrentMotionAnim->InteractionAnims.Num());
			for (int32 i = 0; i < CurrentMotionAnim->InteractionAnims.Num(); i++)
			{
				if (CurrentMotionAnim->InteractionAnims[i])
				{
					if (CurrentMotionAnim->InteractionAnims[i]->GetSkeleton())
					{
						InteractionSkeletons[i] = CurrentMotionAnim->InteractionAnims[i]->GetSkeleton();
					}
					else
					{
			...
			SetupInteractionAnimatedRenderComponents(InteractionSkeletons);
			InteractionCompNum = InteractionAnimatedRenderComponents.Num();
		}
	}
	
	if (InteractionCompNum > 0)
	{
		TArray<FTransform> InteractionComponentTransforms;
		InteractionComponentTransforms.SetNum(InteractionCompNum);

		for (int i = 0; i < InteractionCompNum; i++) {
			if (CurrentMotionAnim) {
				float time = InteractionAnimatedRenderComponents[i]->GetPosition();
				float sourceTime = AnimatedRenderComponent->GetPosition();
				CurrentMotionAnim->GetInteractionAnimsRootBoneTransforms(InteractionComponentTransforms[i], i, sourceTime);
				InteractionAnimatedRenderComponents[i]->SetWorldTransform(InteractionComponentTransforms[i]);
			}
		}
	}
