	...
	if(UAnimationAsset* AnimAsset = MotionAnimAsset->AnimAsset)
	{
		if (AnimAsset->GetSkeleton()->IsCompatibleForEditor(DebugMeshComponent->GetSkinnedAsset()->GetSkeleton()))
		{
			...
			if (MotionAnimAsset->InteractionAnims.Num() > 0)
			{
				...
				for (int32 i = 0; i < MotionAnimAsset->InteractionAnims.Num(); i++)
				{
					UAnimationAsset* InteractionAnimAsset = Cast<UAnimationAsset>(MotionAnimAsset->InteractionAnims[i]);
					if (InteractionAnimAsset) {
						InteractionDebugMeshComponents[i]->EnablePreview(true, InteractionAnimAsset);
						InteractionDebugMeshComponents[i]->SetAnimation(InteractionAnimAsset);
					}
				}
			}
			return true;
...
void FMotionPreProcessToolkit::CacheTrajectory(const bool bMirrored)
{
	...
	// Cache Interaction Animations Trajectories
	if (MotionAnim->InteractionAnims.Num() > 0)
	{
		InteractionCachedTrajectorys.SetNum(MotionAnim->InteractionAnims.Num());
		for (int32 i = 0; i < MotionAnim->InteractionAnims.Num(); i++)
		{
			if (MotionAnim->InteractionAnims[i]) {
				InteractionCachedTrajectorys[i].Empty(FMath::CeilToInt(MotionAnim->InteractionAnims[i]->GetPlayLength() / 0.1f) + 1);
...
void FMotionPreProcessToolkit::DrawCachedTrajectoryPoints(FPrimitiveDrawInterface* DrawInterface)
{
	...
	// Draw Interaction Animations Trajectory
	...
	{
		const UMotionAnimObject* MotionAnim = ActiveMotionDataAsset->GetEditableSourceAnim(CurrentAnimIndex, CurrentAnimType);
		bool bMirrored = ViewportPtr->IsMirror() && (MotionAnim ? MotionAnim->bEnableMirroring : false);
		// 添加或删除了交互动画，要更新CachedTrajectory
		if (MotionAnim->InteractionAnims.Num() != InteractionCachedTrajectorys.Num()) {
			CacheTrajectory(bMirrored);
		}
