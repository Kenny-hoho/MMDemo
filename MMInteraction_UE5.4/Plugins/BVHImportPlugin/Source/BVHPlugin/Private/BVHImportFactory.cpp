...
UAnimSequence* UBVHImportFactory::ImportAnimation(USkeleton* Skeleton, UObject* Outer, FBVHImporter* Importer)
{
	...
	// XC: Extract Root Motion
	const FName RootName = RefSkeleton.GetBoneName(0);

	FRawAnimSequenceTrack RootRawTrack;
	RootRawTrack.PosKeys.Empty();
	RootRawTrack.RotKeys.Empty();
	RootRawTrack.ScaleKeys.Empty();

	int32 NumofKeys = BvhFile->GetNumFrame();
	TArray<FTransform> NewHipTransArray;
	NewHipTransArray.Reserve(NumofKeys);

	for (int32 FrameIdx = 0; FrameIdx < NumofKeys; ++FrameIdx) {
		FTransform OriginRootTransformLocal = RefSkeleton.GetRefBonePose()[0];
		FTransform OriginHipTransformLocal = BvhFile->GetTransform(FrameIdx, BvhFile->GetJoint(0)->index);
		...
		RootRawTrack.ScaleKeys.Add(FVector3f(NewRootTransformLocal.GetScale3D()));
		RootRawTrack.PosKeys.Add(FVector3f(NewRootTransformLocal.GetTranslation()));
		RootRawTrack.RotKeys.Add(FQuat4f(NewRootTransformLocal.GetRotation()));
		...
		**/
		...
		NewHipTransArray.Add(NewHipTransformLocal);
	}
	Controller.AddBoneTrack(RootName);
	Controller.SetBoneTrackKeys(RootName, RootRawTrack.PosKeys, RootRawTrack.RotKeys, RootRawTrack.ScaleKeys);
	...
	for (int32 j = 0; j < BvhFile->GetNumJoint(); ++j)
	{
		...
		if (BoneTreeIndex != INDEX_NONE)
		{
			...
			for (int32 FrameIdx = 0; FrameIdx < BvhFile->GetNumFrame(); ++FrameIdx)
			{
				...
				if (JointIdx == BvhFile->GetJoint(0)->index) {
					LocalTransform = NewHipTransArray[FrameIdx];
				}
				else {
					LocalTransform = BvhFile->GetTransform(FrameIdx, JointIdx);
				}
